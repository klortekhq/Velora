...
assert.match(appSource, /function loadPersonFilmography\(personId, startIndex, works, totalCount\)/);
assert.match(appSource, /loadPersonFilmography\(personId, nextStart, combined, reportedTotal\)/);
assert.match(appSource, /combined\.length >= maxItems/);
assert.match(appSource, /reportedTotal > 0 \? nextStart >= reportedTotal : page\.length < pageSize/);
