...
object MpvShaderManager {
    ...
    // Shader Names
    const val SHADER_HDR_BOOST = "velora_hdr_boost.glsl"
    const val SHADER_CAS = "velora_cas.glsl"
    const val SHADER_ADAPTIVE_SHARPEN = "velora_adaptive_sharpen.glsl"
    const val SHADER_VIBRANCE = "velora_vibrance.glsl"
    const val SHADER_DEBAND = "velora_deband_simple.glsl"
    const val SHADER_DYN_TONEMAP = "velora_dyn_tonemap.glsl"
    ...
    fun getShadersForProfile(context: Context, profile: ShaderProfile): List<String> {
        ...
        when (profile) {
            ...
            ShaderProfile.HdrBoostPlus -> {
                ...
                // Assuming com.klortek.velora.jellyfin.AppSettings is available (imported in file line 2/3?)
                ...
                // Optional shader profiles remain disabled by default to preserve fidelity.
